...
void addEvaluation()
{
    ...
    int selectedTeamID;
    cout << "\nEnter Team ID: ";
    selectedTeamID = getValidInt();


    int teamIndex = findTeamIndexById(selectedTeamID);
    ...
    string rangeErrorMsg = "Error: Score must be between 0 and 10.\n";

    do
    {
        cout << "\nEnter Innovation Score (0-10): ";
        innovation = getValidFloat();
        if (innovation < 0 || innovation > 10)
            cout << rangeErrorMsg;
    } while (innovation < 0 || innovation > 10);

    do
    {
        cout << "Enter Technical Score (0-10): ";
        technical = getValidFloat();
        if (technical < 0 || technical > 10)
            cout << rangeErrorMsg;
    } while (technical < 0 || technical > 10);

    do
    {
        cout << "Enter Presentation Score (0-10): ";
        presentation = getValidFloat();
        if (presentation < 0 || presentation > 10)
            cout << rangeErrorMsg;
    } while (presentation < 0 || presentation > 10);

    //  Build and store the Evaluation struct
    Evaluation e;
    e.EvaluationID = evaluationsCount + 1;
    ...
    e.TotalScore = innovation + technical + presentation;
    ...
    cout << "  Judge  : " << judges[judgeIndex].JudgeName << "\n"
         << "  Team   : " << teams[teamIndex].TeamName << "\n"
         << "  Total  : " << e.TotalScore << " / 30\n";
}